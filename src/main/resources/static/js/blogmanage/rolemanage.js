$(function () {
    var vue = new Vue({
        ...
        methods: {
            init: function (that) {
                that.$options.methods.methodpost(that, 0, "/api/rolemanage/roleInit", {});
            },
            openPop: function (e) {
                vue.roleid = "";
                vue.rolename = "";
            ...
            editRoleClick: function (e, index) {
                var data = vue.personList[index];
                vue.roleid = data.id;
                vue.rolename = data.name;
                vue.description = data.description;
                vue.popupsShow = true;
            },
            delRoleClick: function (e, index) {
                var data = vue.personList[index];
                var roleid = data.id;
                Swal({
                    title: '你确定?',
                    ...
                    if (result.value) {
                        vue.$options.methods.methodpost(vue, 3, "/api/rolemanage/delRole", {
                            roleid: roleid
                        });
                    }
                })
            ...
            itemClick: function (e, index) {
                ...
                vue.$options.methods.methodpost(vue, 1, "/api/rolemanage/nodeInfoById", {
                    roleid: data.id
                });
            },
            faClick: function () {
            ...
            saveClick: function (e) {
                ...
                vue.$options.methods.methodpost(vue, 2, "/api/rolemanage/saveNodeQx", {
                    roleid: vue.chooseData.id,
                    nodelist: JSON.stringify(vue.nodeData)
                });
            },
            popsaveClick: function (e) {
                var rolename = vue.rolename.trim();
                var description = vue.description.trim();
                ...
                for (var i = 0; i < vue.personList.length; i++) {
                    if (strEquals(vue.personList[i].name, rolename) &&
                        (strIsEmpty(vue.roleid) || strEquals(vue.roleid, 0) || strEquals(vue.roleid, "0") ||
                            !strEquals(vue.roleid, vue.personList[i].id))) {
                        Swal({
                            type: 'error',
                            title: '错误提示',
                            text: '角色名称重复!'
                ...
                if (strIsEmpty(roleid)) {
                    roleid = 0;
                }

                vue.$options.methods.methodpost(vue, 3, "/api/rolemanage/roleEdit", {
                    id: roleid,
                    name: rolename,
            ...
            refreshClick: function (e) {
                ...
                vue.$options.methods.methodpost(vue, 0, "/api/rolemanage/roleInit", {}, function () {
            ...
            filldata: function (that, data, mode, callback) {
                switch (mode) {
                    case 0:
                        var jsData = data.resultData.roleData;
